...
int netlist_to_gate_only(Netlist *dest, Netlist *netlist, int component_id) {
    ...
    while (node_ptr!=NULL) {
        ...
        while (_comp != NULL) {
            ...
            for (int i=0; i<comp->_inputc; i++)  {
                ...
                if (map->type == SUBSYS_COMP) {
                    ...
                    if (cmap->prototype->type == SUBSYSTEM) {
                        ...
                        // find the output mapping for the output we are interested in
                        char *nested_map = subsystems[map->index]->output_mappings[map->out_index];

                        // put it in the input list
                        inputs[i] = malloc(strlen(nested_map)+1); // +1 for the null byte
                        strncpy(inputs[i], nested_map, strlen(nested_map)+1);
                    }
            ...
            fprintf(stderr, "inputs for U%d, which is a %s are the following: ", comp->id, comp->prototype->subsys->name);
            for (int i=0; i<comp->_inputc; i++) {
...
void lib_to_file(Netlist *lib, char *filename, char *mode) {
    ...
    for (Node *n = lib->contents; n!=NULL; n=n->next) {
        ...
        if (n->std->type == GATE) {
            ...
            for (Node *n_n=n->std->subsys->components; n_n!=NULL; n_n=n_n->next) {
                ...
                for (int i=0; i<comp->_inputc; i++) {
                    ...
                    if (m->type == SUBSYS_INPUT) {
                        ...
                        b = n->std->subsys->inputs[m->index];
    
                    } else if (m->type == SUBSYS_COMP) {
                        
                        // we will need to crate a new string (that contains the component's id,
                        // not just the output name), so we need to allocate memory for it, and set
                        // the flag so we know that we need to free it afterwards
                        f = 1;

                        // find the node of the component that the mapping refers to
                        Node *rtcn = move_in_list(m->index, n->std->subsys->components);    // referred-to component node
                        ...
                        if (rtcn->comp->prototype->type == GATE) {
                            ...
                            // allocate as much memory as we need
                            b = malloc(strlen(COMP_ID_PREFIX)+digits(rtcn->comp->id)+1+strlen(rtcn->comp->prototype->subsys->outputs[m->out_index])+1);
                            
                            // wrtie the component id and the output name in the string (Uxx_yy)
                            sprintf(b, "%s%d_%s", COMP_ID_PREFIX, rtcn->comp->id, rtcn->comp->prototype->subsys->outputs[m->out_index]);
                        }
                    }
...
void netlist_to_file(Netlist *netlist, char *filename, char *mode) {
    ...
    for(Node *n=netlist->contents; n!=NULL; n=n->next) {
        s2f(n->subsys, fp);
