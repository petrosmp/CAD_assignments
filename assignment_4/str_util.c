...
int list_concat(char*** l1, int l1_c, char **l2, int l2_c) {
    ...
    if (l2==NULL || ((*l1)==NULL && l1_c!=0)) {     // l1 can be null (empty), l1_c just has to be 0
        return NARG;
    }
    ...
    for (i=0; i<l2_c; i++) {
        ...
        // check that the item that will be appended exists
        if (l2[i] == NULL) return NARG;

        // allocate memory for the new element
        (*l1) = realloc((*l1), sizeof(char*)*(l1_c + i+1));   // l1's pointers, i+1 new ones
        (*l1)[l1_c+i] = malloc(strlen(l2[i])+1);

        // insert the new element
        strncpy((*l1)[l1_c+i], l2[i], strlen(l2[i])+1);
